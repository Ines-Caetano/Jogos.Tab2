...
const vertex_shader=`varying vec2 vUv;
	...
	var initScene, initEventHandling, render, createTower, criarRect, loader,
		renderer, scene, dir_light, am_light, camera,
		table, blocks = [], rectan = [],  table_material, block_material, intersect_plane, cone, rect, 
	...
	initScene = function() {
		...
		table_material = Physijs.createMaterial(
			new THREE.MeshLambertMaterial({ map: loader.load( 'images/color.jpg' )}),
			.9, // high friction
			.2 // low restitution
		);
		...
		block_material.map.repeat.set( 5, 5 );
...
createTower = (function() {
		var block_length = 4, block_height = 4, block_width = 4, block_offset = 1,
		...
		return function() {
			...
			for ( i = 0; i < rows; i++ ) {
				for ( j = 0; j < 1; j++ ) {
					...
					block.position.x = block_offset * j - ( block_offset * 1 / 2 - block_offset / 2 );
					block.position.z = block_offset * j - ( block_offset * 1 / 2 - block_offset / 2 );
